...
class FourierImageInputAdapter(nn.Module):
    ...
    def __init__(
        ...
        if learn_pos_feats:
            ...
            # torch < 2.0.0
            x_pos, y_pos = torch.meshgrid(x, y)
...
class TPPInputAdapter(nn.Module):
    def __init__(
        self,
        embed_size: int = 64,
    ):
        super().__init__()
        self.timeinterval_param_mu_embedding = nn.Linear(1, embed_size)
        self.t_embedding = nn.Linear(1, embed_size)

    def forward(self, param_type: torch.Tensor, param_timeinterval: torch.Tensor, history_embedding: torch.Tensor, t: torch.Tensor) -> Tensor:
        param_type = param_type.squeeze(-2)
        interval_emb = self.timeinterval_param_mu_embedding((2*param_timeinterval-1).unsqueeze(-1)).squeeze(-2)
        t_emb = self.t_embedding((2 * t - 1).unsqueeze(-1)).squeeze(-2)
        # output = inp_emb + pos_emb + t_emb
        output = torch.cat((param_type, interval_emb, t_emb, history_embedding), dim=1)
        return output

class TPPOutputAdapter(nn.Module):
    def __init__(self, input_height: int, event_num: int):
        super().__init__()
        self.event_num = event_num
        self.output_type_projection = nn.Linear(
            input_height, event_num
        )
        self.output_timeinterval_projection = nn.Linear(
            input_height, 1
        )

    def forward(self, inp: torch.Tensor) -> torch.Tensor:
        output_type = self.output_type_projection(inp)
        output_timeinterval = self.output_timeinterval_projection(inp)
        return output_type.reshape(
            output_type.size(0), self.event_num
        ), output_timeinterval.reshape(
            output_timeinterval.size(0), 1
        )